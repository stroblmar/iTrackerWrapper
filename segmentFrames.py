# ================================================
# Script to segment frames for each task for each subject in the cibim trial.
# =============================================================
# Configure Script
import os
os.chdir("/workspace/iTrackerAnalysis/benchmarkTrial/rocheData2/data")
...
# =============================================================
subjectNamesList = [fName for fName in os.listdir(".") if os.path.isdir(fName)]
taskNameList = ["calibration","originalFace","enlargedFace","circle"]
# subjectNamesList = ["subject14","subject15"]
subjectNamesList = ["subject16","subject17","subject18"]

subjectName = "olli"
currTaskName = taskNameList[0]

for subjectName in subjectNamesList:
    print("------------------------------------")
    print("Processing files for %s"%(subjectName))
    for currTaskName in taskNameList:
        print("- Task: %s" % (currTaskName))
        # Set up folders to hold segmentations
        currFrameDir = subjectName + "/" + currTaskName + "/allFrames/frames"
        leftEyeDir = subjectName + "/" + currTaskName + "/allFrames/leftEye/"
        rightEyeDir = subjectName + "/" + currTaskName + "/allFrames/rightEye/"
        faceDir = subjectName + "/" + currTaskName + "/allFrames/face/"
        faceGridDir = subjectName + "/" + currTaskName + "/allFrames/faceGrid/"
        notSegmentedDir = subjectName + "/" + currTaskName + "/allFrames/notSegmented/"
